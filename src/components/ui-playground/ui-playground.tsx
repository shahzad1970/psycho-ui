        ...
        return (
            <Host>
                ...
                {this.doc.components.filter(c => c.tag == this.tag).map(c => {
                    return [
                        <ui-layout type="column" gap="medium">
                            <ui-heading size="large">
                                {c.docsTags.filter(t => t.name == "title").map(t => t.text)}
                            </ui-heading>
                            <ui-paragraph>
                                {c.docsTags.filter(t => t.name == "description").map(t => t.text)}
                            </ui-paragraph>
                            <ui-heading size="large">Interactive Component Example</ui-heading>
                            ...
                            <ui-layout type="row" gap="large">
                                    <ui-layout  padding="1.5em" elevation="xx-small" gap="small" type="column" align="start" ref={(e) => this.codeElement = e}
                            ...
                            <ui-heading size="large">Component Properties</ui-heading>
                            <ui-paragraph>
                                Psycho UI Web components are driven by attributes. In order to reduce psychological fatigue on developers, attribute
                                naming and usage has been kept to a minimum so that a developer can easily remember attributes and attribute values.
                           </ui-paragraph>
                            {c.props.map(p => {
                                return [
                                    ...
                                    <ui-heading size="medium">Allowed values</ui-heading>,
